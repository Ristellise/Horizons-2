...
class _slider(widget.Widget):
    ...
    def event(self, e):
            ...
            if 1 in e.buttons and self.container.myfocus is self:
                if self.grab is not None:
                    ...
                    if self.orient == _SLIDER_HORIZONTAL:
                        d = (r.w - self.size)
                        if d != 0:
                            self.value = self.grab_value + ((self.max - self.min) * rel[0] / d)
                    else:
                        d = (r.h - self.size)
                        if d != 0:
                            self.value = self.grab_value + ((self.max - self.min) * rel[1] / d)
                else:
                    x, y, adj = e.pos[0], e.pos[1], 1
        ...
        self.pcls = ""
        if self.container.myhover is self:
            self.pcls = "hover"
        if self.container.myfocus is self and 1 in pygame.mouse.get_pressed():
            self.pcls = "down"
