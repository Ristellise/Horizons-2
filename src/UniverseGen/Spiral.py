...
if HSB == 0:
    NUMHUB = random.randrange(1, 100)
elif HSB == 1:
    NUMHUB = random.randrange(100, 1000)
elif HSB == 2:
    NUMHUB = random.randrange(1000, 100000)
elif HSB == 3:
    NUMHUB = random.randrange(100000, 1000000)
elif HSB == 4:
    NUMHUB = random.randrange(1000000, 2000000)

logging.debug(NUMHUB)
...
NUMARMS = random.randint(2, 12)

ARMROTS = random.uniform(0.2, 2)

ARMWIDTH = (360.0 / NUMARMS) / 1.5
...
def generatestars():
    ...
    while i < NUMDISK:
        ...
        # Puts a star at a given distance into an arm:
        # First, it wraps the star round by the number of rotations specified.
        # multiplying the distance by the number of rotations the rotation is
        # proportional to the distance from the center, to give curvature.
        ...
        # Replace if/elif logic with a simple lookup.
